$(() => {
    ...
    $(document).on("click", ".btn-remove-size", function () {  // xóa dòng size
        let id = $(this).data("id");
        removeSize(sizes, id);
        appendSizesToForm();
    });

    $(document).on("click", ".btn-add-size", function () {  // thêm dòng
        console.log('Adding dòng');
        addSize();
        appendSizesToForm();
    });

    $(document).on("keyup", ".input-size", function () {  // nhập size
        let id = $(this).data("id");
        let size = $(this).val();
        let index = getSizeIndex(sizes, id);
        if (index >= 0) {
    ...
    // $(document).on("keyup", ".input-quantity", function () {  // nhập quantity
    //     let id = $(this).data("id");
    ...
    //     appendSizesToForm();
    // });
    $(document).on("change", ".input-quantity", function () {  // nhập quantity 
        let id = $(this).data("id");
        let quantity = $(this).val();
        let index = getSizeIndex(sizes, id);
